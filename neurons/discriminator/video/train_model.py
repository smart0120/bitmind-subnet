...
def load_video_from_data(data: Any) -> str:
    """Load video from various formats (bytes, dict, path). Returns path to video file."""
    if isinstance(data, str):
        # Check if it's a base64 encoded video
        if len(data) > 1000 and not (data.startswith('http') or '/' in data or '\\' in data):
            try:
        ...
        # Try common keys
        for key in ['video', 'path', 'file_path', 'file', 'bytes']:
            if key in data:
                return load_video_from_data(data[key])
        # Use first value
        first_val = list(data.values())[0]
        return load_video_from_data(first_val)
    else:
        raise ValueError(f"Unknown video data type: {type(data)}")

# ---------------------------
# Dataset Classes
# ---------------------------
class VideoDataset(Dataset):
    """PyTorch Dataset for loading videos."""
    
    def __init__(self, video_data: List[Any], labels: List[int], img_size: Tuple[int, int], num_frames: int = 8):
    ...
    def __getitem__(self, idx):
        ...
        try:
            # Load video path
            video_path = load_video_from_data(data)
...
def load_hf_dataset_from_parquet(
    ...
    """
    Load video dataset from Hugging Face by reading parquet files directly.
    ...
    try:
        ...
        for parquet_file in parquet_files:
            ...
            try:
                ...
                try:
                    ...
                    # Find video column
                    video_col = None
                    for col in ['video', 'path', 'file_path', 'file', 'video_path', 'bytes']:
                        if col in df.columns:
                            video_col = col
                            break
                    
                    if video_col is None:
                        # Try to find any column with video-like data
                        for col in df.columns:
                            if any(k in col.lower() for k in ['video', 'bytes', 'content', 'data']):
                                video_col = col
                                break
                    
                    if video_col is None:
                        print(f"    ⚠️  No video column found in {parquet_file}")
                        continue
                    ...
                    for _, row in df.iterrows():
                        ...
                        try:
                            vid_data = row[video_col]
                            
                            if pd.isna(vid_data) or vid_data is None:
                                continue
                            
                            # Handle different data types
                            if isinstance(vid_data, (bytes, bytearray)):
                                ...
                                # Extract bytes or path from dict
                                for key in ['bytes', 'video', 'data', 'content', 'path']:
                                    if key in vid_data:
                                        video_data.append(vid_data[key])
                                        count += 1
                                        break
                            
                            if count % 5000 == 0:
                                print(f"    Loaded {count} videos so far...")
                        
                        except Exception as e:
                            continue
                    
                    print(f"    Loaded {len(video_data) - (count - df.shape[0])} videos from {parquet_file}")
        ...
        print(f"Loaded {len(video_data)} total videos from {dataset_name}")
        return video_data
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            data = data.permute(0, 2, 1, 3, 4).to(device, non_blocking=True)
...
def main(args):
    ...
    train_dataset = VideoDataset(train_data, train_labels, img_size, args.num_frames)
    val_dataset = VideoDataset(val_data, val_labels, img_size, args.num_frames)
    ...
    if torch.cuda.is_available():
        # Estimate memory per sample (num_frames * 4 channels, 256x256)
        img_size = args.img_size[0] * args.img_size[1]
        bytes_per_sample = args.num_frames * img_size * 4 * 4  # num_frames * 4 channels * 4 bytes (float32)
        ...
        if torch.cuda.get_device_properties(0).total_memory > 70 * 1024**3:  # > 70GB
            suggested_batch = min(128, int(80 / estimated_batch_memory * args.batch_size))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch-size", type=int, default=32, 
                        help="Batch size (default: 32 for videos on large GPU, increase for 80GB GPU)")
