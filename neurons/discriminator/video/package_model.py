"""
Script to package trained video model for submission.
...
def package_video_model(
    model_dir: str,
    output_zip: str = None,
    model_name: str = "video_detector"
):
    """
    Package video model files into a zip for submission.
    ...
    # Create zip file
    print(f"Packaging video model from: {model_dir}")
    ...
    print(f"\n✓ Video model packaged successfully: {output_zip.resolve()}")
    print(f"  File size: {output_zip.stat().st_size / (1024*1024):.2f} MB")
    print(f"\nReady to submit with:")
    print(f"  gascli d push --video-model {output_zip.name} --wallet-name <your_wallet> --wallet-hotkey <your_hotkey>")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Package video model for submission")
    parser.add_argument(
        "model_dir",
        type=str,
    ...
    parser.add_argument(
        "--name",
        type=str,
        default="video_detector",
        help="Model name for zip file (default: video_detector)"
    )
    
    args = parser.parse_args()
    
    try:
        package_video_model(args.model_dir, args.output, args.name)
